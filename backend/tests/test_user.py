...
def test_get_users_info_with_params(client, sample_data):
    response = client.get(
        "/api/v1.0/users/users?fields=username&sort=-id&page=2&limit=2"
    )
    response_data = response.get_json()
    assert response.status_code == 200
    assert response.headers["Content-Type"] == "application/json"
    ...
    assert response_data["data"] == [{"username": "username"}, {"username": "username"}]
...
def test_create_github_user(client, token, github_user_info):
    ...
    response_data = response.get_json()
    expected_result = {"data": {**github_user_info, "id": 1}}
...
def test_create_github_user_info_invalid_data(client, token, data, missing_field):
    response = client.post(
        "/api/v1/users/users", json=data, headers={"Authorization": f"Bearer {token}"}
    )
    response_data = response.get_json()
...
def test_create_github_user_info_missing_token(client, github_user_info):
    response = client.post("/api/v1/users/users", json=github_user_info)
    response_data = response.get_json()
